...
function render_client() {
    ...
    if(toRender_client == false){ return; }
    console.log('render');
    ...
    for(i = 0; i < client_inUse.length; i++){
        ...
        if(client_inUse[i] == true){
            ...
            doRender_clientSauce(render_clientSauce, inRender_client, render_wishState);
        }else{
...
function render_clientPatience(){
    ...
    if(toRender_patience == false){return;}
...
function doRender_clientWish(render_clientWish, inRender_client,render_wishState){
...
function doRender_clientSauce(render_clientSauce, inRender_client, render_wishState){

    //Check if the sauce is defined, in case is not, will be false, so no render.
    if(inRender_client[4] == false){return;}
...
function doRender_clientPatience(i){
    ...
    let patiencePercentage = (client_patienceActual[i] * 100) / client_patienceSet[i];
